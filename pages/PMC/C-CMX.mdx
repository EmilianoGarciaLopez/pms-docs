...
PDO3: Default set to 20V, 1A minimum
PDO2: Default set to 15V, 1.75A minimum
PDO1: PDO1 is 5V only, therefore it is disabled.

The order the PDOs are listed is also the precedence of the PD negotiation. STUSB will request PDO3 first, and if available will take PDO3. If using a 20V-capable charger but a lower bus voltage is desired (15V, 9V), the NVMe must be reflashed.
...
![STUSB4500 interface pinout](/images/pmc/USB-PDinterface_pindiagram.png)
...
Arduino UNO R3 pins -> PMC-C-CMX USB-PD interface
GND - > G
3V3 - > V+
A4 - > SC
A5 - > SD
